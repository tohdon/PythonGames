...
while True:
    ...
    while y< height - MIN_Y_INCREASE:
        ...
        for x in range(width):
            canvas[(x,y)] =BLACK
        y += random.randint(MIN_Y_INCREASE,MAX_Y_INCREASE)
    numberOfReactanglesToPaint = numberOfSegmentsToDelete - 3
    numberOfSegmentsToDelete = int(numberOfSegmentsToDelete * 1.5)
    print(str(len(canvas)))
        ...
        while True:
            ...
            if orientation == 'vertical':
                for changey in (-1, 1):
                    ...
                    while 0 < y < height -1:
                        y += changey
                        if canvas[(startx - 1, starty )]== BLACK and canvas[(startx + 1, starty )]== BLACK:
                            break
                        elif ((canvas[(startx -1 , starty )] == WHITE and canvas[(startx + 1, starty )]== BLACK) or
                            (canvas[(startx -1 , starty )]== BLACK and canvas[(startx + 1, starty )] == WHITE)):
    ...
    for i in range(numberOfReactanglesToPaint):
        while True:
            startx = random.randint(1, width -2)
            starty = random.randint(1,  height -2)
            if canvas[(startx, starty)] != WHITE:
    ...
    for y in range(height):
        ...
        print()
    try:
        input('Please enter for another work of art or Ctrl-C to quit')
